    ...
    methods: {
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将永久删除该物品, 是否继续?', '提示', {
          ...
          this.removeRowInTheStore(row, index)
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
      ...
      removeRowInTheStore(row) {
        const products = this.products.filter(p => p.id !== row.id)
        utils.storageSetPromise('products_list', products).then(() => {
      ...
      confirmCheckout() {
        ...
        this.handleConfirmCheckout(this.checkoutProductRow, () => {
          this.removeRowInTheStore(this.checkoutProductRow)
        })
      },
